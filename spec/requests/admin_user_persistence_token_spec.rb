...
RSpec.describe "admin user persistence token", type: :request, csrf: false do
  ...
  let(:login_params) do
    { email: "admin@petitions.gov.je", password: "L3tme1n!" }
  end
  ...
  def new_browser
    open_session do |s|
      s.reset!
      s.host! "moderate.petitions.gov.je"
      s.https!
    end
  end
  ...
  context "when a new session is created" do
    it "logs out existing sessions" do
      ...
      expect(s1.response.headers["Location"]).to eq("https://moderate.petitions.gov.je/admin")

      s2 = new_browser
      s2.post "/admin/user_sessions", admin_user_session: login_params

      expect(s2.response.status).to eq(302)
      expect(s2.response.headers["Location"]).to eq("https://moderate.petitions.gov.je/admin")

      s1.get("/admin")

      expect(s1.response.status).to eq(302)
      expect(s1.response.headers["Location"]).to eq("https://moderate.petitions.gov.je/admin/login")
    end
  end
  ...
  context "when a session is destroyed" do
    it "resets the persistence token" do
      ...
      expect(s2.response.status).to eq(302)
      expect(s2.response.headers["Location"]).to eq("https://moderate.petitions.gov.je/admin/login")
    end
  end

  context "when a session is stale" do
    before do
      host! "moderate.petitions.gov.je"
      https!
    end
